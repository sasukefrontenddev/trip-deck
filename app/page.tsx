...
import {
  ...
  CalendarDaysIcon, CheckCircleIcon, ClipboardDocumentCheckIcon, CloudArrowDownIcon,
...
const countries: { name: CountryName; code: string; dates: string; city: string; vibe: string }[] = [
  ...
  { name: 'Indonesia', code: 'ID', dates: '30 Aug–3 Sep · 4 nights', city: 'Bali', vibe: 'Temples, beaches and sunsets' },
];
...
const starterAttractions: Attraction[] = [
  ...
  { id: 'a5', country: 'Indonesia', city: 'Bali', name: 'Ubud day trip', category: 'Culture', saved: true },
  { id: 'a6', country: 'Indonesia', city: 'Bali', name: 'Sunset beach visit', category: 'Beach', saved: true },
];
...
type Tab = 'overview' | 'itinerary' | 'documents' | 'expenses' | 'stays' | 'toolkit';
...
export default function Home() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!(await getAll('attractions')).length) for (const attraction of starterAttractions) await put('attractions', attraction);
      await refresh();
    })();
  ...
  return <main>
    ...
    <header className="topbar glass"><div><span className="eyebrow">GROUP TRAVEL OS</span><h1>TripDeck<span>.</span></h1></div><div className={`status ${online ? 'online' : 'offline'}`}><WifiIcon/>{online ? 'Online' : 'Offline · local mode'}</div></header>
    ...
    <nav className="tabs glass">{(['overview', 'itinerary', 'documents', 'expenses', 'stays', 'toolkit'] as Tab[]).map(t => <button key={t} onClick={() => setTab(t)} className={tab === t ? 'active' : ''}>{t}</button>)}</nav>
    ...
    <AnimatePresence mode="wait">
      {tab === 'overview' && <motion.section className="content" key="overview" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="country-grid">{countries.map((c, i) => <motion.article className="country-card" key={c.code} whileHover={{ y: -8 }}><span className="number">0{i + 1}</span><div className={`flag f${c.code}`}>{c.code}</div><h4>{c.name}</h4><p>{c.dates}</p><small>{c.city} · {c.vibe}</small></motion.article>)}</div>
        <div className="two-col"><section className="panel glass"><div className="panel-title"><h3>Bookings</h3><button onClick={() => setShowAdd(true)}><PlusIcon/></button></div>{bookings.map(b => <div className="booking" key={b.id}><div className="booking-icon">{b.type === 'flight' ? <PaperAirplaneIcon/> : <MapPinIcon/>}</div><div><b>{b.title}</b><span>{b.subtitle}</span></div><div className="booking-right"><b>{formatDate(b.date)}</b><span>{b.confirmation}</span></div></div>)}<button className="gmail"><CloudArrowDownIcon/> Import booking emails <span>Connect Gmail when online</span></button></section>
