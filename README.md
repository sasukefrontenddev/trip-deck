...
## Live Trip Mode (V31)
Trip Deck now includes an active travel-day command center that sits on top of the existing app without replacing any current workflow.

- Detects the active trip country and shows the destination-local clock.
- Surfaces the next itinerary activity with a live countdown.
- Calculates a `Leave now` / `Leave in …` cue from the activity time and stored commute estimate, with a built-in 10-minute buffer.
- Shows the next flight, terminal, gate and live status in the same travel view.
- Adds a Schedule Radar that scans itinerary spacing for time clashes and transfers that are tighter than the stored commute time.
- Shows the current day's load as Easy, Balanced or Packed using stop count and planned commute minutes.
- Includes Rescue Mode with local emergency numbers, saved hotel information, next flight and direct private-document access.
- Generates a small Offline Pocket Brief containing the essential flight, hotel, group, next-plan and emergency details. It can be copied or downloaded as a text file.
- Adds one-tap hotel directions from the live command center.
