...
function HotelCard({ hotel, onSaved }: { hotel: HotelStay; onSaved: () => void }) {
  ...
  async function submit(formData: FormData) {
    ...
    try { const response=await fetch('/api/travel',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({origin:airport,destination:`${name}, ${address}, ${city}, ${hotel.country}`,country:hotel.country})}); const route=await response.json(); if(response.ok){distanceKm=route.distanceKm;transferMinutes=route.drivingMinutes;} else setRouteMessage(route.error||'Could not calculate airport transfer.'); } catch { setRouteMessage('Could not calculate airport transfer.'); }
...
function FlightForm({ online, onSaved }: { online: boolean; onSaved: () => void }) {
  ...
  async function submit(formData: FormData) {
    ...
    const departure = normalizeWallClock(f?.departure?.scheduledTime?.local || String(formData.get('departureTime')));
