...
import {
  ...
  TrashIcon, UserGroupIcon, WifiIcon, ArrowTopRightOnSquareIcon, ClockIcon, ListBulletIcon
...
const COUNTRY_ORDER: Record<CountryName, number> = { Malaysia: 0, Singapore: 1, Indonesia: 2 };
const cleanItineraryCopy = (value = '') => value
  .replace(/^\s*\d{1,2}:\d{2}\s*/i, '')
  .replace(/\s+[–—-]{1,2}\s*$/g, '')
  .replace(/\s{2,}/g, ' ')
  .trim();
const cleanItineraryLocation = (value = '', title = '') => {
  const cleaned = cleanItineraryCopy(value);
  const cleanTitle = cleanItineraryCopy(title);
  if (!cleaned) return 'Location not specified';
  if (cleaned.toLowerCase() === cleanTitle.toLowerCase() || cleaned.length > 120) {
...
export default function Home() {
  ...
  const tripStartMs = +new Date('2026-08-21T21:40:00');
  const tripEndMs = +new Date('2026-09-04T23:59:00');
  const daysToTrip = Math.max(0, Math.ceil((tripStartMs - now) / 86_400_000));
  const tripPhase = now < tripStartMs ? `${daysToTrip} day${daysToTrip === 1 ? '' : 's'} to go` : now <= tripEndMs ? 'Trip in progress' : 'Trip completed';
  const nextItineraryItem = useMemo(() => items
    .filter(item => +new Date(`${item.date}T${item.time || '00:00'}`) >= now)
    .sort((a,b) => +new Date(`${a.date}T${a.time || '00:00'}`) - +new Date(`${b.date}T${b.time || '00:00'}`))[0], [items, now]);
  const nextHotel = useMemo(() => hotels
    .filter(hotel => +new Date(`${hotel.checkOut}T23:59`) >= now)
    .sort((a,b) => +new Date(`${a.checkIn}T00:00`) - +new Date(`${b.checkIn}T00:00`))[0], [hotels, now]);
  const totalSpendAED = useMemo(() => expenses.reduce((sum, expense) => {
    if (Number.isFinite(expense.aedAmount)) return sum + Number(expense.aedAmount);
    if (expense.currency === 'AED' && Number.isFinite(expense.amount)) return sum + Number(expense.amount);
    return sum;
  }, 0), [expenses]);
  const nextFlightHotel = useMemo(() => {
    if (!nextBooking) return nextHotel;
    return hotels.find(hotel => hotel.country === nextBooking.country) || nextHotel;
  }, [hotels, nextBooking, nextHotel]);
  const nextFlightPlan = useMemo(() => {
    if (!nextBooking) return nextItineraryItem;
    const flightTime = +new Date(nextBooking.arrivalTime || nextBooking.revisedArrivalTime || nextBooking.date);
    return items
      .filter(item => item.country === nextBooking.country && +new Date(`${item.date}T${item.time || '00:00'}`) >= flightTime)
      .sort((a,b) => +new Date(`${a.date}T${a.time || '00:00'}`) - +new Date(`${b.date}T${b.time || '00:00'}`))[0] || nextItineraryItem;
  }, [items, nextBooking, nextItineraryItem]);
  const travelDaySoon = !!nextBooking && (+new Date(nextBooking.date) - now) <= 48 * 60 * 60 * 1000;
  const travelDayTravelers = nextBooking ? TRAVELER_COUNT_BY_COUNTRY[nextBooking.country] : 6;
  ...
  return <main>
    ...
    <AnimatePresence mode="wait">
      {tab === 'overview' && <motion.section className="content" key="overview" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <section className="trip-pulse glass"><div className="trip-pulse-head"><div><span className="eyebrow">TRIP PULSE</span><h3>{tripPhase}</h3></div><span className={`pulse-status ${online ? 'ok' : ''}`}>{online ? 'Cloud connected' : 'Offline mode'}</span></div><div className="trip-pulse-grid"><button onClick={() => setTab('bookings')}><PaperAirplaneIcon/><div><span>NEXT FLIGHT</span><b>{nextBooking ? `${nextBooking.departureAirportCode || '---'} → ${nextBooking.arrivalAirportCode || '---'}` : 'Add your next flight'}</b><small>{nextBooking ? `${nextBooking.flightNumber || nextBooking.airline || 'Flight'} · ${formatWallClock(nextBooking.date)}` : 'Keep live status, gate and terminal together'}</small></div></button><button onClick={() => setTab('itinerary')}><ClockIcon/><div><span>NEXT PLAN</span><b>{nextItineraryItem?.title || 'Build the day-by-day plan'}</b><small>{nextItineraryItem ? `${formatDate(nextItineraryItem.date)} · ${nextItineraryItem.time} · ${nextItineraryItem.location}` : 'Import a PDF or add stops manually'}</small></div></button><button onClick={() => setTab('stays')}><BuildingOffice2Icon/><div><span>STAY</span><b>{nextHotel?.name || 'Add accommodation'}</b><small>{nextHotel ? `${nextHotel.city} · ${formatDate(nextHotel.checkIn)} → ${formatDate(nextHotel.checkOut)}` : 'Keep address and airport transfer ready'}</small></div></button><button onClick={() => setTab('toolkit')}><ListBulletIcon/><div><span>READINESS</span><b>{checklist.length ? Math.round(prepDone/checklist.length*100) : 0}% ready</b><small>{checklist.length-prepDone} task{checklist.length-prepDone===1?'':'s'} remaining</small></div></button></div></section>
        <section className={`travel-day-companion glass ${travelDaySoon ? 'urgent' : ''}`}>
          <div className="travel-day-head"><div><span className="eyebrow">TRAVEL DAY COMPANION</span><h3>{nextBooking ? `${nextBooking.departureAirportCode || '---'} → ${nextBooking.arrivalAirportCode || '---'}` : 'Your next travel day'}</h3><p>{nextBooking ? `${nextBooking.airline || 'Flight'} ${nextBooking.flightNumber || ''} · ${formatWallClock(nextBooking.date)} · ${travelDayTravelers} travelers` : 'Add a flight and TripDeck will assemble the important travel-day details here.'}</p></div><span className={`travel-day-badge ${travelDaySoon ? 'urgent' : ''}`}>{travelDaySoon ? 'TRAVEL DAY SOON' : 'NEXT LEG'}</span></div>
          <div className="travel-day-grid">
            <button onClick={() => setTab('bookings')}><PaperAirplaneIcon/><div><span>FLIGHT READY</span><b>{nextBooking?.status || 'Saved itinerary'}</b><small>{nextBooking ? `Terminal ${nextBooking.terminal || '—'} · Gate ${nextBooking.gate || '—'}${nextBooking.checkInDesk ? ` · Desk ${nextBooking.checkInDesk}` : ''}` : 'Live terminal, gate and status appear here'}</small></div></button>
            <button onClick={() => setTab('stays')}><BuildingOffice2Icon/><div><span>ON ARRIVAL</span><b>{nextFlightHotel?.name || 'Add your stay'}</b><small>{nextFlightHotel?.address || 'Keep the hotel address ready for immigration, taxi or Grab'}</small></div></button>
            <button onClick={() => setTab('itinerary')}><ClockIcon/><div><span>FIRST PLAN</span><b>{nextFlightPlan?.title || 'No plan yet'}</b><small>{nextFlightPlan ? `${formatDate(nextFlightPlan.date)} · ${nextFlightPlan.time} · ${nextFlightPlan.location}` : 'Your first activity after arrival will appear here'}</small></div></button>
            <button onClick={() => setTab('expenses')}><BanknotesIcon/><div><span>TRIP SPEND</span><b>AED {totalSpendAED.toLocaleString(undefined,{maximumFractionDigits:0})}</b><small>{expenses.length ? `${expenses.length} expense${expenses.length===1?'':'s'} tracked · ~AED ${(totalSpendAED/Math.max(1,6)).toLocaleString(undefined,{maximumFractionDigits:0})} per traveler` : 'Track spending and group settlements as you go'}</small></div></button>
          </div>
          <div className="travel-day-actions"><button className="secondary small" onClick={() => setTab('documents')}><ShieldCheckIcon/> Travel documents</button><button className="secondary small" onClick={() => setTab('toolkit')}><ClipboardDocumentCheckIcon/> Prep checklist</button>{nextFlightHotel?.address && <a className="primary small" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(nextFlightHotel.address)}`} target="_blank" rel="noopener noreferrer"><MapPinIcon/> Hotel directions</a>}</div>
        </section>
      ...
      {tab === 'stays' && <motion.section className="content" key="stays" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><div className="section-heading"><div><span className="eyebrow">HOTELS & THINGS TO DO</span><h3>Stay planner</h3></div></div>
        <div className="hotel-grid">{[...hotels].sort((a,b) => COUNTRY_ORDER[a.country] - COUNTRY_ORDER[b.country]).map(hotel => <HotelCard key={hotel.id} hotel={hotel} onSaved={refresh}/>)}</div>
      ...
      {tab === 'toolkit' && <motion.section className="content" key="toolkit" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><div className="section-heading"><div><span className="eyebrow">TRAVEL SMARTER</span><h3>Group toolkit</h3></div><button className="secondary small" onClick={exportTrip}><ArrowDownTrayIcon/> Backup data</button></div><div className="tool-grid"><section className="panel glass"><div className="panel-title"><h3>Checklist & packing</h3><ClipboardDocumentCheckIcon/></div><form className="checklist-add" onSubmit={async e => { e.preventDefault(); const title=newChecklistTitle.trim(); if(!title) return; await put('checklist',{id:`custom-${crypto.randomUUID()}`,title,category:'Custom / packing',done:false}); setNewChecklistTitle(''); await refresh(); }}><input value={newChecklistTitle} onChange={e=>setNewChecklistTitle(e.target.value)} placeholder="Add packing item or trip task"/><button className="primary small" type="submit"><PlusIcon/> Add</button></form><div className="checklist">{checklist.map(item => <div className={`checklist-row ${item.done ? 'done' : ''}`} key={item.id}><label><input type="checkbox" checked={item.done} onChange={async () => { await put('checklist', { ...item, done: !item.done }); await refresh(); }}/><div><b>{item.title}</b><span>{item.category}</span></div></label>{item.id.startsWith('custom-') && <button className="checklist-delete" title="Delete custom item" onClick={async()=>{await remove('checklist',item.id);await refresh();}}><TrashIcon/></button>}</div>)}</div></section><section className="panel glass emergency"><div className="panel-title"><h3>Emergency card</h3><PhoneIcon/></div><div className="emergency-list"><div><b>Malaysia</b><span>Emergency: 999</span></div><div><b>Singapore</b><span>Police: 999 · Fire/Ambulance: 995</span></div><div><b>Indonesia</b><span>Emergency: 112</span></div></div></section></div></motion.section>}
    </AnimatePresence>
    ...
    <AnimatePresence>{documentPreview && <motion.div className="document-preview-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={closeDocumentPreview}><motion.div className="document-preview-modal" initial={{opacity:0,y:18,scale:.99}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:10,scale:.99}} onMouseDown={e=>e.stopPropagation()}>
      ...
      <footer className="document-preview-actions"><div><a className="secondary preview-open" href={documentPreview.url} target="_blank" rel="noopener noreferrer"><span className="preview-action-icon"><ArrowTopRightOnSquareIcon/></span><span>Open in new tab</span></a><a className="primary preview-download" href={documentPreview.url} download={documentPreview.doc.name}><span className="preview-action-icon"><ArrowDownTrayIcon/></span><span>Download</span></a></div></footer>
